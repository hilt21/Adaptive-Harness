...
class StorageManager:
    ...
    def __init__(
        ...
        *,
        ...
        if repository_id is None:
            ...
            self.root = resolve_project_data(data_root, repository_id, None)
    ...
    def register(self, item: StorageItem) -> None:
        with bound_project_data_lock(
            ...
            items[:] = [
                existing for existing in items if existing["id"] != item.id
            ]
    ...
    def status(self) -> StorageStatus:
        with self._bound_operation():
            ...
            return StorageStatus(
                ...
                active_count=sum(
                    item["task_state"] in _PROTECTED_TASK_STATES
                    for item in items
                ),
        ...
        with self._bound_operation():
            ...
            for item in self._load()["items"]:
                ...
                cutoff = current - timedelta(
                    days=_RETENTION_DAYS[item["category"]]
                )
                if created > cutoff:
                    continue
                path = self.root.joinpath(*PurePosixPath(item["path"]).parts)
    ...
    def _bound_operation(self) -> Any:
        return bound_project_data_lock(
            self.root,
            self._storage_locator,
            force_user_data=self._force_user_data,
        )
        ...
        with bound_project_data_lock(
            ...
            for item_id, relative_path in planned.items():
                ...
                if item["path"] != relative_path:
                    raise ValueError(
                        f"storage item changed after review: {item_id}"
                    )
                path = self.root.joinpath(*PurePosixPath(relative_path).parts)
                if path.is_symlink():
                    raise ValueError(
                        f"refusing to prune symlink: {relative_path}"
                    )
